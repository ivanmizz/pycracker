...
# a password cracker to figure out passwords that were hashed using SHA-1.
'''this program only works if you have the sha-1 hash of the password, however it can be edited to crack any hash'''  # for converting passwords and guesses to hashes
...
chars = \
    '1234567890abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZZ'
charslen = len(chars)
...
answer = input('>')
if answer.lower() == 'crack':  # for some reason this mode won't work with the online interpreter, but it will with my offline python interpreter.
    ...
    password = input('>')
    passwordHash = hashlib.sha1(password.encode('utf-8')).hexdigest()  # converts your entered password into SHA-1
    print (passwordHash)
clear()
print ('starting...')
                                ...
                                while i8 <= charslen - 1:
                                    while i9 <= charslen - 1:
                                        while i10 <= charslen - 1:
                                            ...
                                            loops += 1
                                            hashed = \
    hashlib.sha1(guess.encode('utf-8')).hexdigest()
...
if error == False:
    ...
    print ('Tried', loops, 'combinations in', round(timeNow
           - startTime, 2), 'seconds')
    if timeNow == startTime:
        timeNow += 1
    print ('An average of', round(loops / (timeNow - startTime), 2),
           'pw/s')
else:
